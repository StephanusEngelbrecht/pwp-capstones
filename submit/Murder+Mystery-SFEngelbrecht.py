...
class TextSample:
    ...
    def __repr__(self):
        return "The author's name is {a} and the average sentence length is {b}".format(a = self.author, b = self.average_sentence_length)
...
def prepare_text(text):
    ...
    text2 = []
    for word in text1:
        word = word.replace(" ", "").replace('!','').replace('?','').replace("\"","").lower().replace(",","").replace(".","")
        text2.append(word)
    return text2
...
def build_frequency_table(corpus):
    ...
    for element in corpus:
        ...
        for x in corpus:
            if element == x:
                counter += 1
        frequency_table[element] = counter
    return frequency_table

c = ['do', 'you', 'see', 'what', 'i', 'see']
d = build_frequency_table(c)
print(d)
...
def ngram_creator(text_list):
    ...
    for i in range(len(text_list)-1):
        out.append(text_list[i] + " " + text_list[i+1])
    return out
...
t1 = {'what': 1, 'you': 1, 'see': 2, 'i': 1}
t2 = {'whats': 1, 'yous': 1, 'see': 2, 'is': 1}
x  =frequency_comparison(t1, t2)
print(x)
...
def find_text_similarity(TextSample1, TextSample2):
    ...
    return (ngram_similarity+word_count_similarity+sentence_length_similarity)/3
...
print("Lily's note has a similarity score of {xx} to the murder note".format(xx=find_text_similarity(murderer_sample,lily_sample)))
print("Myrtle's note has a similarity score of {xx} to the murder note".format(xx=find_text_similarity(murderer_sample,myrtle_sample)))
print("Gregg's note has a similarity score of {xx} to the murder note".format(xx=find_text_similarity(murderer_sample,gregg_sample)))
...
print("The murderer is Gregg T Fishy")
